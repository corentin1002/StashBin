...
- A specific date and time can be chosen instead of a preset lifetime: picking "Specific date and time…" reveals a date field, whose value is read in the visitor's own timezone and sent as a Unix timestamp. The instant is refused rather than corrected when it is already past or beyond year 9999 — a secret outliving what its author asked for is the one mistake here that cannot be taken back. Two lines of defence keep a past date from being encrypted at all: the field's own `min`, and a check in the script for the browsers that render `datetime-local` as a plain text field. The inventory shows every date in UTC and now says so.
