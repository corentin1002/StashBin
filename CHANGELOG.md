...
- Interface multilingue : français et anglais, choisis d'après l'en-tête `Accept-Language` du navigateur, avec repli sur `default_locale` (`en` par défaut, surchargeable par `STASHBIN_LOCALE`) : un visiteur dont aucune langue demandée n'est traduite reçoit l'anglais. `src/lang/fr.php` reste la langue de référence, celle dont une traduction partielle emprunte ses clés manquantes — les deux rôles sont distincts. Une étiquette régionale retombe sur sa langue (`fr-CA` → `fr`). Le paramètre `?lang=` force la langue d'une page, sans cookie ni session. Ajouter une langue consiste à déposer un fichier dans `src/lang/` : le code liste ce dossier. Les réponses portent `Vary: Accept-Language`.
