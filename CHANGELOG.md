...
- A specific date and time can be chosen instead of a preset lifetime. Picking "Specific date and time…" reveals two native fields — a date, which browsers open as a calendar, and a time, which they open as a clock or a spinner. They start on today at the time it is, so that only what usually changes has to be touched, and are refreshed whenever the instant they hold has fallen behind the clock. Their value is read in the visitor's own timezone and sent as a Unix timestamp.
  ...
  The instant is refused rather than corrected when it is already past or beyond year 9999 — a secret outliving what its author asked for is the one mistake here that cannot be taken back. Two lines of defence keep a past date from being encrypted at all: the date field's own `min`, and a check in the script for the browsers that validate nothing. The inventory renders every date in the reader's own timezone: the server writes UTC, since it cannot know where anyone is, and the browser rewrites it. Served without JavaScript, the dates stay readable and say they are UTC.
