...
- Jeu de test complet (`tests/run.sh`) : 114 tests répartis en quatre suites — unitaire (fonctions de `src/bootstrap.php`), API (règles métier), sécurité (garanties annoncées par le README) et navigateur (cryptographie de bout en bout et parcours d'interface dans Chromium). Le lanceur démarre une instance neuve, joue les suites et détruit tout ; son code de sortie vaut 0 si et seulement si tout passe. Aucune dépendance Composer : les assertions et le client HTTP tiennent dans `tests/lib.php`.
