...
```bash
./tests/run.sh                    # 175 tests, quelques minutes — à lancer avant de valider
...
- **Aucun texte d'interface en dur.** Toute chaîne vue par un visiteur passe par `t()` et vit dans `src/lang/`. `src/lang/fr.php` fait référence : une clé qui n'y figure pas n'a pas de repli. Les commentaires des fichiers de langue restent en français, y compris dans `en.php`.
