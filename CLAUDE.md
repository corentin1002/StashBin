...
```bash
./tests/run.sh                    # 244 tests, quelques minutes — à lancer avant de valider
...
- **Les instants sont écrits en UTC et localisés par le navigateur.** `time_tag()` pose l'instant deux fois — dans `datetime`/`data-stamp` pour le script, en texte UTC pour qui n'en a pas — et `stashbin.js` réécrit le contenu. Ne remplacez pas ça par une conversion côté serveur : il ne connaît pas le fuseau du lecteur. C'est aussi pourquoi `secrets.php` charge le script.
